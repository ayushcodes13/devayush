                ...
                >
                    ...
                    <div className="relative flex flex-col space-y-3">
                        <div className="flex items-center justify-between">
                            ...
                            <span
                                className={cn(
                                    "text-[10px] font-mono font-medium px-2 py-0.5 rounded-lg backdrop-blur-sm",
                                    "bg-white/5 text-muted-foreground",
                                    "transition-colors duration-300 group-hover:bg-white/10 group-hover:text-foreground"
                                )}
                            >
                                {item.status || "Active"}
                            </span>
                        ...
                        <div className="space-y-2">
                            <h3 className="font-medium text-foreground tracking-tight text-[15px]">
                                {item.title}
                                <span className="ml-2 text-xs text-muted-foreground/60 font-mono font-normal">
                                    {item.meta}
                                </span>
                            </h3>
                            <p className="text-sm text-muted-foreground leading-snug font-[400]">
                                {item.description}
                            </p>
                        </div>

                        <div className="flex items-center justify-between mt-2">
                            <div className="flex items-center space-x-2 text-xs text-muted-foreground/50">
                                {item.tags?.map((tag, i) => (
                                    <span
                                        key={i}
                                        className="px-2 py-0.5 rounded-md bg-white/5 backdrop-blur-sm transition-all duration-200 hover:bg-white/10"
                                    >
                                        #{tag}
                                    </span>
                                ))}
