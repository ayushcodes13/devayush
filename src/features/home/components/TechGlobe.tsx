...
const techItems = [
  ...
  { name: "LangChain", icon: "lobehub:langchain" },
  { name: "LangGraph", icon: "lobehub:langgraph" },
  { name: "LlamaIndex", icon: "lobehub:llamaindex" },
  { name: "PostgreSQL", icon: "postgresql" },
  { name: "FAISS", icon: "meta" },
  { name: "OpenAI", icon: "lobehub:openai" },
  { name: "Anthropic", icon: "lobehub:anthropic" },
  { name: "FastAPI", icon: "fastapi" },
  { name: "Docker", icon: "docker" },
  { name: "AWS", icon: "lobehub:aws" },
];
    ...
    <group position={point.position}>
      ...
      <Html center distanceFactor={6} style={{ pointerEvents: "none", userSelect: "none" }}>
        ...
        >
          {point.icon ? (
            <img 
              src={point.icon.startsWith("lobehub:") ? `https://unpkg.com/@lobehub/icons-static-svg@latest/icons/${point.icon.split(":")[1]}.svg` : `https://cdn.simpleicons.org/${point.icon}/currentColor`} 
              alt="" 
              className={point.icon.startsWith("lobehub:") ? "w-3 h-3 opacity-60 dark:invert invert" : "w-3 h-3 opacity-60 dark:invert"} 
            />
          ) : (
...
const TechGlobe = ({ compact = false }: { compact?: boolean }) => {
  ...
  return (
    <div style={{ width: "100%", height, position: "relative" }}>
      <Canvas camera={{ position: [0, 0, compact ? 9 : 8], fov: 45 }}>
        <GlobeScene />
      </Canvas>
